...
const collectObjPath = (currentPath, nextStep, index) => {
  ...
  console.info("collectObjPath", currentPath, nextStep, index)
...
const createEvent = (field, v, userId, date, objPath, index) => {
  console.info("createEvent for", field, v, userId, date, objPath, index)
...
const processAuditDiffs = (delta, nextEvent, userId, date, pathArg) => {
  console.info("processing delta", delta)
  console.info("processing nextEvent", nextEvent)
  ...
  Object.keys(delta)?.forEach((k, index) => {
    ...
    if (Array.isArray(delta[k])) {
      ...
      // check first element of array for modification operators: "~", "+", "-"
        ...
        console.info("before update forEach", delta[k][1])
        Object.keys(delta[k][1])?.forEach((l) => {
          console.info("currently inspecting", l)
          const objPathArr = copy(objPath)
          if (Array.isArray(delta[k][1][l])) {
            console.info("delta[k][1][l]", delta[k][1][l])
            console.info("nextEvent", nextEvent, k)
            console.info("nextEvent[k]", nextEvent[k])
            //TODO: nextEvent is undefined
            tempEvents.push(
              ...processAuditDiffs(
                delta[k][1][l],
                nextEvent[k] ? nextEvent[k] : nextEvent,
                userId,
                date,
